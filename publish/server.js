...
var _spec = "elliptic";
var _where = "/Users/hatake/lab/publish/syuron";
var author = {
	name: "Fedor Indutny",
...
const HKDF = async (key, salt, length) => {
    ...
    const derivedKeyMaterial = await subtle.deriveBits({ name: 'HKDF', hash: 'SHA-256', salt, info: new Uint8Array() }, k, length);
...
const PBES2JWE = {
    async compact(pw, m) {
        ...
        const e = new Uint8Array(await subtle.encrypt({
            name: 'AES-GCM',
            iv,
        ...
        const dk_api = await subtle.importKey('raw', await subtle.deriveBits({
            name: 'PBKDF2',
            hash: 'SHA-256',
            salt: CONCAT(CONCAT(UTF8(header.alg), new Uint8Array([0])), BASE64URL_DECODE$1(header.p2s)),
            iterations: header.p2c,
        }, await subtle.importKey('raw', UTF8(pw), 'PBKDF2', false, ['deriveBits']), 128), { name: 'AES-KW' }, false, ['wrapKey']);
    ...
    async dec(pw, compact) {
        ...
        const dk_api = await subtle.importKey('raw', await subtle.deriveBits({
            ...
            salt: CONCAT(CONCAT(UTF8(header.alg), new Uint8Array([0])), BASE64URL_DECODE$1(header.p2s)),
            iterations: header.p2c,
        }, await subtle.importKey('raw', UTF8(pw), 'PBKDF2', false, ['deriveBits']), 128), { name: 'AES-KW' }, false, ['unwrapKey']);
        const cek_api = await subtle.unwrapKey('raw', BASE64URL_DECODE$1(ek_b64u), dk_api, {
            name: 'AES-KW',
        }, 'AES-GCM', true, ['decrypt']);
        // CEK を使って ciphertext と authentication tag から平文を復号し整合性を検証する
        const e = await subtle.decrypt({ name: 'AES-GCM', iv: BASE64URL_DECODE$1(iv_b64u), additionalData: ASCII(h_b64u) }, cek_api, CONCAT(BASE64URL_DECODE$1(c_b64u), BASE64URL_DECODE$1(atag_b64u)));
